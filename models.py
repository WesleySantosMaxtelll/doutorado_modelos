...
class BartUserTwitter(nn.Module):
    def __init__(self, device, output_size=2):
        super(BartUserTwitter, self).__init__()
        self.device = device
        # pre_trained = 'neuralmind/bert-base-portuguese-cased'
        # pre_trained = 'pablocosta/bertweet-br-base-uncased'
        # self.bert = BertModel.from_pretrained(pre_trained)
        self.bert = BartModel.from_pretrained("facebook/bart-large", forced_bos_token_id=0)
        self.hidden_size = self.bert.config.hidden_size
        self.rnn = nn.LSTM(input_size=self.hidden_size, hidden_size=100,
                           num_layers=4, bidirectional=True)
        self.sample_size = 10
...
class BertFTLW(nn.Module):
    def __init__(self, device, output_size=2):
        super(BertFTLW, self).__init__()
        self.device = device
        # pre_trained = 'neuralmind/bert-base-portuguese-cased'
        pre_trained = "pablocosta/bertabaporu-base-uncased"
        self.bert = BertModel.from_pretrained(pre_trained)
        # self.bert = LlamaModel.from_pretrained("pablocosta/llama-7b", use_auth_token="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", load_in_8bit=True, device_map='auto')
        self.hidden_size = self.bert.config.hidden_size
        self.rnn = nn.LSTM(input_size=self.hidden_size, hidden_size=128,
                           num_layers=2)

        self.fc_out3 = nn.Linear(128, output_size)

    def forward(self, tokens, masks, predictions):
        outputs = []
        # iterate in the batch through all sequences
        for token, mask, p in zip(tokens, masks, predictions):
            encoded = self.bert(input_ids=token.reshape(1, -1), attention_mask=mask.reshape(1, -1))['last_hidden_state']
            out, hiden = self.rnn(encoded)
            out = self.fc_out3(out[:, -1, :])
            out = torch.softmax(out, -1)
            outputs.append((out * p).sum())
        outputs = torch.stack(outputs)
        return torch.stack([1 - outputs, outputs], dim=1).float()


class UserTimeseries(nn.Module):
    def __init__(self, device, output_size=2):
        super(UserTimeseries, self).__init__()
        self.device = device
        self.hidden_size = 2
        self.rnn = nn.LSTM(input_size=self.hidden_size, hidden_size=5,
                           num_layers=4, bidirectional=True)
        self.sample_size = 10
        self.fc_out2 = nn.Linear(40, 2)

    def forward(self, tss):
        outputs = []
        # iterate in the batch through all sequences
        for ts in tss:
            out, hiden = self.rnn(ts.unsqueeze(0))
            # x = torch.stack(hiden).flatten()
            # x = F.relu(self.fc_out1(x))
            out = out.permute(1, 2, 0)  # Reshape for max pooling (batch_size, hidden_size * num_directions, seq_len)
            x = self.fc_out2(hiden[0].permute(1, 2, 0)[-1].flatten())
            x = torch.softmax(x, -1)
            outputs.append(x)
